...
open (NODEFILE, $f_node =~ /\.gz$/? "gzip -dc $f_node |" : $f_node =~ /\.xz$/? "xz -dc $f_node |" : $f_node)  || die "I can't open node file $f_node\n";
open (PLFILE,   $f_pl =~ /\.gz$/? "gzip -dc $f_pl |" : $f_pl =~ /\.xz$/? "xz -dc $f_pl |" : $f_pl)    || die "I can't open pl   file $f_pl\n";
open (SOLFILE,  $f_sol =~ /\.gz$/? "gzip -dc $f_sol |" : $f_sol =~ /\.xz$/? "xz -dc $f_sol |" : $f_sol)   || die "I can't open sol  file $f_sol\n";
open (NETFILE,  $f_net =~ /\.gz$/? "gzip -dc $f_net |" : $f_net =~ /\.xz$/? "xz -dc $f_net |" : $f_net)   || die "I can't open net  file $f_net\n";
